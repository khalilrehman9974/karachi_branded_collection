...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1>List of Delivery Charges</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-left">
                            <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                            <li class="breadcrumb-item active">List of Delivery Charges</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
        ...
        <section class="content">
            ...
            <div class="row">
                ...
                <div class="btn-group float-right" role="group">
                    <a href="{{ route('delivery-charges.create') }}"  class="form-control btn btn-primary"  type="button">
                        Create Customer
                    </a>
                </div>
            <br>
                    ...
                    <table class="table table-striped">
                        ...
                        @if(!$deliveryCharges->isEmpty())
                            @foreach($deliveryCharges as $charges)
                                <tr class="delivery-charges{{ $charges->id }}">
                                    <td>{{ $charges->id }}</td>
                                    <td>{{ $charges->shipment_type }}</td>
                                    <td>{{ $charges->zoneName }}</td>
                                    <td> {{ $charges->fuel_percentage }}</td>
                                    <td> {{ $charges->gst_percentage }}</td>
                                    <td> {{ $charges->additional_kg_charges }}</td>
                                    <td class="project-actions float-right">
                                        <a class="btn btn-primary btn-sm" target="_blank"
                                           href="{{ route('delivery-charges.view',$charges->id) }}"
                                           id="view-package1" title="View">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a class="btn btn-primary btn-sm"
                                           href="{{ route('delivery-charges.edit', $charges->id) }}"
                                           data-id="{{ $charges->id }}" title="Edit">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>
                                        <a class="btn btn-primary btn-sm delete"
                                           href="javascript:void(0)"
                                           data-id="{{ $charges->id }}" title="Delete">
                        ...
                        @else
                            <tr>
                                <td colspan="6"><strong>No delivery charges data</strong></td>
                            </tr>
                        @endif
                        </tbody>
                    </table>
            ...
            <div class="float-left">
                <b>Total Records: </b> {{ $deliveryCharges->total() }}
            </div>
            <div class="float-right">
                <nav>
                    <ul>

                        {!! $deliveryCharges->setPath('')->appends(\Illuminate\Support\Facades\Request::query())->links() !!}
                    </ul>
                </nav>
            </div>
            <!-- /.card -->
    ...
    <!-- /.content-wrapper -->
    <script src="{{ asset('js/deliveryCharges.js') }}"></script>
    <script src="{{ asset('js/common.js') }}"></script>
    <script>
        // global routes configuration object
        var config = {
            routes: {
                deleteDeliveryCharges: "{{ url('delivery-charges/delete') }}",
            },
            totalRecords: {{  $deliveryCharges->total() }}
        };
    </script>
@endsection
