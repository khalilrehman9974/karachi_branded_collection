    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1> {{ !empty($sale) ? 'Edit Sale Entry' : 'Create New Sale' }}</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-left">
                            <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('sale.sales') }}">List of all
                                    Sale</a>
                            </li>
                            <li class="breadcrumb-item active"> {{ !empty($sale) ? 'Edit Sale' : 'Create New Sale' }}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
        ...
        @endif
        <form method="post"
              action="{{ !empty($sale) ? route('sale.update') : route('sale.store') }}" id="frmSale">
            @csrf
            ...
            <section class="content">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="form-group col-md-4">
                                        ...
                                        <div class="input-group date" id="date"
                                            ...
                                            <input type="text" class="form-control datetimepicker-input date"
                                                   ...
                                                   data-target="#sale-date"
                                                   value="{{ empty( $sale->date) ? null :  \Illuminate\Support\Carbon::parse(  $sale->date)->format('d-m-Y')}}"}}/>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <select class="form-control select2" style="width: 100%;"
                                            ...
                                            @foreach($customers as $key=>$value)
                                                <option value="{{ $key }}" {{!empty($sale) && ($sale->customer_id === $key) ? "selected" : ''}} {{($key == old('customer_id')) ? 'selected' : ''}}>{{ $value }}</option>
                                            @endforeach
                                        </select>
                                        @error('customer_id')
                                        <div class="error-message">{{ $message }}</div>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <select class="form-control select2" style="width: 100%;"
                                            ...
                                            @foreach($brands as $key=>$value)
                                                <option value="{{ $key }}"  {{!empty($sale) && ($sale->brand_id === $key) ? "selected" : ''}} {{($key == old('brand_id')) ? 'selected' : ''}}>{{ $value }}</option>
                                            @endforeach
                                        </select>
                                        @error('brand_id')
                                        <div class="error-message">{{ $message }}</div>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <input type="text" class="form-control" name="tracking_number" id="tracking_number" value="{{@$sale->tracking_number}}" />
                                    </div>
                                    <div class="form-group col-md-4">
                                        <label>Remarks</label>
                                        <textarea class="form-control" name="remarks" id="remarks" value="">{{ @$sale->remarks }}</textarea>
                                    </div>
                                ...
                                <div class="card">
                                    <div class="table-responsive">
                                            <table class="table table-striped">
                                                ...
                                                <tbody id="tableBody">
                                                @if(!empty($sale))
                                                    @foreach($saleDetails as $productDetail)
                ...
                <div class="row clearfix">
                    <div class="col-12" style="text-align: right;">
                        <input type="button" value="{{ !empty($sale) ? 'Update' : 'Save' }}" id="save" class="btn btn-success"
                               style="background-color: #3c8dbc">
                        <a href="{{ route('sale.sales') }}" class="btn btn-secondary ">Cancel</a>
                    </div>
                </div>
            </section>
        </form>
