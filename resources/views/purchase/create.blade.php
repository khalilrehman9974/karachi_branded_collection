...
@section('content')
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1> {{ !empty($purchase) ? 'Edit Purchase Entry' : 'Create New Purchase' }}</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-left">
                            <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('purchase.purchases') }}">List of all
                                    Purchase</a>
                            </li>
                            <li class="breadcrumb-item active"> {{ !empty($purchase) ? 'Edit Purchase' : 'Create New Purchas' }}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
        ...
        @endif
        <form method="post"
              action="{{ !empty($purchase) ? route('purchase.update') : route('purchase.store') }}" id="frmPurchase">
            @csrf
            ...
            <section class="content">
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <div class="input-group date" id="date"
                                            ...
                                            <input type="text" class="form-control datetimepicker-input date"
                                                   ...
                                                   value="{{ empty( $purchase->date) ? null :  \Illuminate\Support\Carbon::parse(  $purchase->date)->format('d-m-Y')}}"}}/>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <select class="form-control select2" style="width: 100%;"
                                            ...
                                            @foreach($parties as $key=>$value)
                                                <option value="{{ $key }}" {{!empty($purchase) && ($purchase->party_id === $key) ? "selected" : ''}} {{($key == old('party_id')) ? 'selected' : ''}}>{{ $value }}</option>
                                            @endforeach
                                        </select>
                                        @error('party_id')
                                        <div class="error-message">{{ $message }}</div>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <select class="form-control select2" style="width: 100%;"
                                            ...
                                            @foreach($brands as $key=>$value)
                                                <option value="{{ $key }}"  {{!empty($purchase) && ($purchase->brand_id === $key) ? "selected" : ''}} {{($key == old('brand_id')) ? 'selected' : ''}}>{{ $value }}</option>
                                            @endforeach
                                        </select>
                                        @error('brand_id')
                                        <div class="error-message">{{ $message }}</div>
                                ...
                                <div class="card">
                                    <div class="table-responsive">
                                            <table class="table table-striped">
                                                ...
                                                <tbody id="tableBody">
                                                @if(!empty($purchase))
                                                    @foreach($purchaseDetails as $productDetail)
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tr class="tr_clone validator_0">
                                        ...
                                        <td><input type="text" name="totalQty"
                                                   ...
                                                   id="quantity" value="{{!empty($purchase) ? $purchase->quantity : ''}}"></td>
                                        <td><input type="text" name="totalAmount" id="totalAmount" class="form-control totalAmount"
                                                   value="{{!empty($purchase) ? $purchase->amount : ''}}"
                ...
                <div class="row clearfix">
                    <div class="col-12" style="text-align: right;">
                        <input type="button" value="{{ !empty($purchase) ? 'Update' : 'Save' }}" id="save" class="btn btn-success"
                               style="background-color: #3c8dbc">
                        <a href="{{ route('purchase.purchases') }}" class="btn btn-secondary ">Cancel</a>
                    </div>
                </div>
            </section>
        </form>
    ...
    <script>
        ...
        var config = {
            routes: {
                getBrandProducts: "{{ url('purchase/get/brand-products') }}",
                getProductDetail: "{{ url('purchase/get/product-detail') }}",
            },
        };
    </script>
@endsection
