/*
* Delete Purchase call.
* @var id
* @var ajax_url
* */
$(".delete").on('click', function () {
    var id = $(this).attr('data-id');
    var ajax_url = config.routes.deletePurchase;
    $.confirm({
        title: 'Delete Purchase!',
        content: 'Sure to delete Purchase?',
        type: 'red',
        typeAnimated: true,
        buttons: {
            tryAgain: {
...
/*
* Get all Purchases for selected brand.
* */
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status == 'success') {
                    ...
                    $(".price_" + row_id).empty();
                    $(".price_" + row_id).val(response.data.price);
...
/!*
* Get lot numbers on selection of item.
* *!/
$('body').on('change', '.item-code', function () {
    var itemCode = $(this).val();
    var $row = jQuery(this).closest("tr");
    $row.find(".item-code").attr("data-item-number", $(this).closest("tr").find(".item-code").val());
    $row.find(".from-serial").attr("data-item", itemCode);
    var row_id = $(this).closest("tr").find(".row_id").val();
    $(".uom_" + row_id).val("");
    $.ajax({
        url: config.routes.getItem,
        type: 'GET',
        dataType: "json",
        data: {'itemCode': itemCode},
        beforeSend: function () {
            $(this).closest("tr").find(".uom").val();
            $(this).closest("tr").find(".item-class").val();
        },
        success: function (response) {
            $(".uom_" + row_id).val(response.uom);
            $(".item_class_" + row_id).val(response.item_class);
        },
        complete: function () {
            $(".item-number_" + row_id).css('display', 'none');
        },
        error: function () {
